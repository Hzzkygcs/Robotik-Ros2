...
DISTANCE_THRESHOLD_TO_GOAL = 0.3
...
class Navigate(Node):
    def __init__(self):
        ...
        self.backward_cooldown = 0  # if it reaches certain number, it will not go backward no mater what
    ...
    def speed_multiplier(self):
        if self.obstacle_distances is None:
            return 1
        min_obstacle_distance  = self.obstacle_distances.min()
        return np.clip(1, 3 * min_obstacle_distance, 2.1)
    ...
    def try_to_skip_visible_goal_points(self, distances=None):
        ...
        for i in range(self.goal_point_index, all_target.shape[0]):
            ...
            too_far = np.linalg.norm(all_target[i, :] - curr_pos) > 4*DISTANCE_THRESHOLD_TO_GOAL
            if vision_blocked and not too_far:
                self.goal_point_index = i
        if before < self.goal_point_index:
    ...
    def robot_go_backward(self, obstacle_left, obstacle_hit_penalty, on_done_callback=None):
        self.get_logger().info(f"GOING BACKWARD. obstacle at the {'left' if obstacle_left else 'right'}")
        def penalty():
            self.backward_cooldown = time.time() + obstacle_hit_penalty
            print(f"CANNOT GO BACKWARD. SETTING COOLDOWN {obstacle_hit_penalty}")
    ...
    def goal_angle(self, relative_to_pov=True, angle_range_start=-math.pi, angle_range_end=math.pi + 0.001, original_goal=False):
        goal_point = self.goal_point if not original_goal else self._goal_point
        dx = goal_point.x - self.robot_pose.x
        dy = goal_point.y - self.robot_pose.y
    ...
    def get_point_relative_to_orientation(self, point):
        dx = point.x - self.robot_pose.x
        dy = point.y - self.robot_pose.y
        distance = np.sqrt(dx ** 2 + dy ** 2)
        alpha = math.atan2(dy, dx) - self.robot_pose.theta + math.radians(90)
        x = distance * math.cos(alpha)
        y = distance * math.sin(alpha)
        return distance, alpha, (x,y)

    def get_absolute_point_from_relative_point(self, point):
        if isinstance(point, Point):
            point = (point.x, point.y)
        alpha = math.atan2(point[1], point[0]) + self.robot_pose.theta - math.radians(90)
        distance = np.sqrt(point[0] ** 2 + point[1] ** 2)
        x = distance * math.cos(alpha)
        y = distance * math.sin(alpha)
        ret = Point()
        ret.x = self.robot_pose.x + x
        ret.y = self.robot_pose.y + y
        return ret
    ...
    def navigate(self):
        ...
        self.backward_cooldown = max(0, self.backward_cooldown - 1)
        if closest_overall is not None:
            ...
            if condition and time.time() >  self.backward_cooldown:
                self.robot_go_backward(obstacle_at_left, 1.5, on_done_callback=self.redo_bfs)


        cmd_vel = Twist()
        variable_multiplier = self.speed_multiplier
        if distance > DISTANCE_THRESHOLD_TO_GOAL:
            cmd_vel.linear.y = 0.6  * SPEED_MULTIPLIER * variable_multiplier
            ...
            cmd_vel.angular.z = 2.0 * theta * (SPEED_MULTIPLIER * variable_multiplier)**0.25
        else:
    ...
    def _get_rotate_in_place(self, max_angle=MAX_ANGLE_DEGREE_TOWARD_GOAL):
        goal_angle_deg = math.degrees(self.goal_angle())
        def rotation():
            deg = math.degrees(self.goal_angle(original_goal=True))
    ...
    def goal_distance(self, original=False):
        goal_point = self.goal_point if not original else self._goal_point
        dx = goal_point.x - self.robot_pose.x
        dy = goal_point.y - self.robot_pose.y
        return np.sqrt(dx ** 2 + dy ** 2)

    def handle_arrived(self):
        if self.goal_distance(original=True) > DISTANCE_THRESHOLD_TO_GOAL:
            return
