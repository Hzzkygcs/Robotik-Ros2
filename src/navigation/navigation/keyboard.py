...
class Keyboard(Node):
    ...
    def update_goal_point_from_keyboard(self):
        ...
        if key == ord('w'):
            self.current_goal_point = [self.position.x, self.position.y + self.movement_distance]
            ...
            self.current_goal_point = [self.position.x - self.movement_distance, self.position.y]
